var gd_goods_view = function () {
    ...
    this.input_count_change = function (inputName, goodsFl) {
        ...
        if (parseInt(nowCnt % salesUnit) > 0) {
            alert(setGoodsNm + __("%1$s개 단위로 묶음 주문 상품입니다.", salesUnit));
            $(inputName).val($(inputName).data('value'));
            return false;
        }
    ...
    this.option_price_display = function (inputName) {
        ...
        if (setOptNo != "" && setOptNo != undefined) {
            if ($(inputName).data('params') == undefined) {
                ...
                if ($('.js-option-select' + setOptNo).length == 1) $('.js-option-select' + setOptNo).html(__("옵션을 선택하세요."));
            }
        }
        ...
        if (setOptionDisplayFl == 's') {
            ...
            $(".js-multi-option .js-option-select").html(__("옵션을 선택하세요."));
            $(".js-multi-option .js-option-select:not(:first)").addClass("disabled");
        }
    ...
    this.option_select = function (inputName, thisKey, stockViewFl) {
        ...
        for (var i = 0; i <= thisKey; i++) {
            ...
            if (optionVal[i] == '') {
                for (var j = (i + 1); j <= optionCnt; j++) {
                    if (j != 0) {
                        $(frmId + ' li.optionNo_' + j + ' .js-option-select').addClass('disabled');
                    }
                }
        ...
        $.post('../goods/goods_ps.php', {
            ...
            for (var i = 0; i <= optionCnt; i++) {
                if (i <= data.nextKey) {
                    $(frmId + ' li.optionNo_' + i + ' .js-option-select' + setOptNo).removeClass('disabled');
                    if (i == data.nextKey) {
                        $(frmId + ' li.optionNo_' + i + ' ul.optlst').html("");

                        nextVal = $(frmId + ' li.optionNo_' + (i - 1) + ' .js-option-select' + setOptNo).data("next-option");
                    ...
                    if (i != 0) {
                        $(frmId + ' li.optionNo_' + i + ' .js-option-select').addClass('disabled');
                    }
                }
            }
    ...
    this.remove_option = function (optionId) {
        ...
        if (ontionCnt == 0 || this.optionStockCount < 2) {
            this.optionStockCount = 0;
        } else {
            this.optionStockCount--;
        }
...
function GoodsController() {
    ...
    GoodsController.prototype.run = function () {
        this.init();
    };

    GoodsController.prototype.init = function () {
        this.initInfoClickEvent();
    };

    GoodsController.prototype.initInfoClickEvent = function () {
        
        $("div#infoDetail_title").click({
            "$contentDiv": $("div#infoDetail_content"),
            "$arrowDiv": $("div#infoDetail_arrow")
        }, goodsController.onclickInfoTab);

        $("div#infoBasic_title").click({
            "$contentDiv": $("div#infoBasic_content"),
            "$arrowDiv": $("div#infoBasic_arrow")
        }, goodsController.onclickInfoTab);

        $("div#infoPackage_title").click({
            "$contentDiv": $("div#infoPackage_content"),
            "$arrowDiv": $("div#infoPackage_arrow")
        }, goodsController.onclickInfoTab);

        
        $("div#infoReview_title").click({
            "$contentDiv": $("div#infoReview_content"),
            "$arrowDiv": $("div#infoReview_arrow")
        }, goodsController.onclickInfoTab);


        $("div#infoQNA_title").click({
            "$contentDiv": $("div#infoQNA_content"),
            "$arrowDiv": $("div#infoQNA_arrow")
        }, goodsController.onclickInfoTab);

        $("div#infoDelivery_title").click({
            "$contentDiv": $("div#infoDelivery_content"),
            "$arrowDiv": $("div#infoDelivery_arrow")
        }, goodsController.onclickInfoTab);

        $("div#infoExchange_title").click({
            "$contentDiv": $("div#infoExchange_content"),
            "$arrowDiv": $("div#infoExchange_arrow")
        }, goodsController.onclickInfoTab);

        $("div#infoRefund_title").click({
            "$contentDiv": $("div#infoRefund_content"),
            "$arrowDiv": $("div#infoRefund_arrow")
        }, goodsController.onclickInfoTab);

        $("div#infoAS_title").click({
            "$contentDiv": $("div#infoAS_content"),
            "$arrowDiv": $("div#infoAS_arrow")
        }, goodsController.onclickInfoTab);
		$("div#review_title").click({
            "$contentDiv": $("div#review_content"),
            "$arrowDiv": $("div#review_arrow")
        }, goodsController.onclickInfoTab);
    };

    GoodsController.prototype.onclickInfoTab = function (e) {
        var $arrowDiv = e["data"]["$arrowDiv"];
        var $contentDiv = e["data"]["$contentDiv"];

        if ($arrowDiv && $contentDiv) {
            if ($contentDiv.css("display") === "none") {
                $contentDiv.slideDown(200);
                $arrowDiv.attr("class", "store_detail_arrow arrow_up");

            } else {
                $contentDiv.slideUp(200);
                $arrowDiv.attr("class", "store_detail_arrow arrow_down");
            }
        }
    };
